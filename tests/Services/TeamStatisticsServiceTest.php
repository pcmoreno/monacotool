...
class TeamStatisticsServiceTest extends TestCase
{
    private TeamStatisticsService $service;

    protected function setUp(): void
    {
        $this->service = new TeamStatisticsService();
    }

    public function test_output_average_returns_zero_when_no_iterations(): void
    {
        $this->assertSame(0.0, $this->service->getOutputAverage(new Team()));
    }

    public function test_output_average_is_correct(): void
    {
        $this->assertSame(20.0, $this->service->getOutputAverage($this->teamWithOutputs(10, 20, 30)));
    }

    public function test_standard_deviation_returns_zero_with_fewer_than_two_iterations(): void
    {
        $this->assertSame(0.0, $this->service->getSampleStandardDeviation(new Team()));
        $this->assertSame(0.0, $this->service->getSampleStandardDeviation($this->teamWithOutputs(5)));
    }
    ...
    public function test_standard_deviation_is_correct(): void
    {
        ...
        $this->assertEqualsWithDelta(3.0277, $this->service->getSampleStandardDeviation($team), 0.0001);
    }
